...
from ..utils.config import cfg    # rm 'lib.', or cfg will be create a new copy
...
from ..rpn.proposal_target_layer import _ProposalTargetLayer
...
class _FPN(nn.Module):
    ...
    def _PyramidRoI_Feat(self, feat_maps, rois, im_info):
        ...
        # print("you are using a wrong fpn file")

        #todo add by yihong
        h = torch.clamp(h, 1,im_info[0][0] -1)
        w = torch.clamp(w, 1,im_info[0][1] -1)
        h[h != h] = 1
        w[w != w] = 1

        roi_level = torch.log(torch.sqrt(
            h * w) / cfg.PYRAM_ROI_C4_SIZE) / np.log(2)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        p6 = self.maxpool2d(self.mrcnn_feature_maps[3])

        rpn_feature_maps = self.mrcnn_feature_maps + [p6]
        mrcnn_feature_maps = self.mrcnn_feature_maps
        self.training = True
        ...
        if training and not cfg.CLASS_AGNOSTIC_BBX_REG:
