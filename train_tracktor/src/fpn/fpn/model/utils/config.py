...
__C.PIXEL_STDVS = np.array([[[1.0, 1.0, 1.0]]])

# For reproducibility
__C.RNG_SEED = 3

# For reproducibility
__C.CLASS_AGNOSTIC_BBX_REG = False
...
def get_output_tb_dir(imdb, weights_filename):
  ...
  """
  outdir = osp.abspath(
      osp.join(__C.ROOT_DIR, 'tensorboard', 'fpn', __C.EXP_DIR, imdb.name))
  if weights_filename is None:
    weights_filename = 'default'
  outdir = osp.join(outdir, weights_filename)
  if not os.path.exists(outdir):
