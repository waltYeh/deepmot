...
def tmp_nms(boxes, scores, overlap=0.5, top_k=6000):
...
class _ProposalLayer_FPN(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(batch_size):
            ...
            # scores_single.add_(3).view(-1, 1)
            # keep_idx_i = nms(torch.cat((proposals_single, scores_single), 1), nms_thresh)
            keep_idx_i = tmp_nms(proposals_single, scores_single.squeeze(1), overlap=nms_thresh)
